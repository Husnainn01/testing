            ...
            <div class="w-100 border p-5">
                @foreach ($option_services as $key => $option_service)
                    <div class="form-check text-start">
                        <input class="form-check-input" name="service_name[{{ $key }}]" type="checkbox"
                            value="{{ $option_service->id }}" id="service_id_{{ $option_service->id }}">
            ...
            document.getElementById('order-select-button').addEventListener('click', function() {
                ...
                checkboxes.forEach(function(checkbox) {
                    ...
                    var rowData = {
                        ...
                        stockId: row.cells[3].textContent.trim(),
                        engine: row.cells[4].textContent.trim(),
                    ...
                    success: function(response) {
                        ...
                        if (response.cities && response.cities.length > 0) {
                            ...
                            $.each(response.cities, function(id, name) {
                                citiesDropdown.append('<option value="' + name.id +
                                    '">' + name.name + '</option>');
                            });
                        } else {
                        ...
                        if (response.ports && response.ports.length > 0) {
                            ...
                            $.each(response.ports, function(id, name) {
                                portDropdown.append('<option value="' + name.id + '">' +
                                    name.name + '</option>');
                            });
                        } else {
