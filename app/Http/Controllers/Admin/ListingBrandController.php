...
class ListingBrandController extends Controller
{
    ...
    public function admin_shippment_show($id)
    {
        ...
        $offers = $shippment_request->offers;
        return view('admin.shippment_request_show', compact('shippment_request','offers'));
    }
    ...
    public function admin_shipping_documents_update(Request $request)
    {
        ...
        foreach ($request->offer_id as $key => $offerId) {
            $offer = Qoute::findOrFail(intval($offerId));
            $offer->status = $request->offer_agreed_status[$key];
            $offer->agreed_price = $request->offer_agreed_price[$key];
            $offer->save();
        }
        ...
        if ($shippment->status == 'approved') {
            foreach ($request->offer_id as $key => $offerId) {
                ...
                $car->listing_stock_status = $request->offer_agreed_status[$key];
                $car->save();
            }
